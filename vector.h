...
namespace OOO {
    ...
    public:
        Vector() : Vector(0, 0, 0) {
            // id
            std::cout << "Vector#" << id << " created by defconstruct" << std::endl;
        }
        Vector(double valueX, double valueY, double valueZ) : x(valueX), y(valueY), z(valueZ), id(_id++) {
            // id
            std::cout << "Vector#" << id << " created by constructor" << std::endl;
        }
        ...
        ~Vector() {
            std::cout << "Vector#" << id << " delited" << std::endl;
        }
...
// Получаем значение Z
        ...
        void normalize();
        bool isZero() const;
        void copy(const OOO::Vector &Obj1);
    };

// надо было добавить & потом все пошло в жепу
...
// Скалярное произвдеине векторов A и B
    ...
    double sinVec(Vector &Obj1, Vector &Obj2);
